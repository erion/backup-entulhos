...
type
  ...
  TPeca = class(TObject)
    ...
    public
      ...
      function GetCodPeca(peca:TPeca):integer;
  ...
  TMovimento = class
    private
      mCodigo:integer;
      mDisponivel: array [1..8,1..8] of integer;  //armazena posicoes [x],[y] disponiveis.
      mImagem: TImage;
    ...
    public
      ...
      function GetPosX(Vmovimento:TMovimento):integer;
      function GetPosY(Vmoviemnto:TMovimento):integer;
  end;
...
function TPeca.GetCodPeca(peca: TPeca): integer;
begin
  result := peca.pCodPeca;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
{
        mImagem.Left := mImagem.Left + (28 * (i-1));
        mImagem.Top := mImagem.Top + (52 * (j-1));


        mImagem.Left := mImagem.Left + (peca.pImagem.Left -23);
        mImagem.Top := mImagem.Top + (peca.pImagem.Top);
}
...
function TMovimento.GetPosX(Vmovimento: TMovimento): integer;
begin
  result := mPosXY[1];
end;

function TMovimento.GetPosY(Vmoviemnto: TMovimento): integer;
begin
  result := mPosXY[2];
end;
