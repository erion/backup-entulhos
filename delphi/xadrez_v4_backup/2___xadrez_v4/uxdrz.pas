...
begin
  if xPecaSelecionada = nil then
    ...
    //criar procedure CancelaMovimento; (destroi imagens de movimento criadas)
  end;

  for i := 1 to IdentificaQTDMov(xPecas[(Sender as TImage).Tag]) do
  begin
    xMovimento[i] := TMovimento.Create(pnlTabuleiro,(Sender as TImage).Tag);
...
begin
  ...
  begin
    ...
    xPecas[i].pImagem.OnClick := MovimentaPeca;
  end;
end;
