<?php
class fManyToManySystem extends Field {
	var $valores;
	var $exibirSelecione;
	var $valor;
	var $classePK;
	...
	function fManyToManySystem($campoFK, $label, $classePK, $labelPK, $classeFK, $labelFK, $visible=true, $valores="", $filtro = array()) {
	...
	function capturarValor($cod = 0) {
		...
		if ( isset( $obj ) ) {
			...
			$query = "SELECT ".$this->campo.", ".$label." FROM ".$obj->tabela . $where . " ORDER BY ".$label;
			...
			if ( $cod )
				$query = "SELECT ".$this->campo." FROM ".$this->getTableName()." WHERE ".$objPK->chave." = " . $cod;
			else
				$query = "SELECT ".$this->campo." FROM ".$this->getTableName();
			$rs = $db->query($query);
