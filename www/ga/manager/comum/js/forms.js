...
/* MicroModulos */
function toggleListagensMicroModulos() {
	if ($('.ativar_listagem').length > 0) {
		var objs = $('.ativar_listagem');
		var url = "";
		for (var i=0; i<objs.length; i++) {
			url = String($(objs[i]).attr('href')).replace('javascript:','');
			eval(url);
		}
	}
}

function toggleView(classe, acao, campo, valor) {
	var field = '#'+campo+'_'+acao;
		if ($(field+":hidden").length > 0) {
			updateView(classe, acao, campo, valor);
		} else {
			$(field).hide();
		}
}
...
function updateView(classe, acao, campo, valor) {
	var field = '#'+campo+'_'+acao;
	var fieldname = $('#'+campo+'_'+acao+'_field').val();
	
	if (valor == '' || valor == "undefined") { valor = 0; }
	
	url = 'actions.php?field=fMicroModulo&method=ajax&classe='+classe+'&campo='+campo+'&acao='+acao+'&valor='+fieldname+'&'+fieldname+'='+valor+'&evento=busca';
		...
		var check = $('input[name="deletar[]"]:checked').size();
			if( check > 0 ) {
				if (confirm('Deseja realmente excluir os dados selecionados?')) {				
					if ( !$(this).parents('div.microform-content') ) {					
						...
						$.post(action, serialize,
								function(d) {
									if( d == "true" ) {
										...
										alert("Não foi possível deletar os registros!");
									}
								}
						);
					}
