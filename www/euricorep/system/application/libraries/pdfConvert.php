...
class pdfConvert {
        ...
        function createMyDocument($filename) { //cria copia do arquivo somente com os dados necessarios ao sistema
            $dados = $this->output();

            $resultLinhas = array();
//            $expLinhasUteis = "/([0-9]{6}) - ([a-zA-Z]).*([0-9a-zA-Z]).*(4.00)$/";
//            $expLinhasUteis = "/([0-9]{6})(.*)(4\.00|[a-zA-Z]|\s).*/";
//            preg_match_all($expLinhasUteis, $dados, $resultLinhas);
//
//            var_dump($resultLinhas);exit;

            $expArtigos = "/([0-9]{6}) - ([a-zA-Z]).*/";
            $expOC = "/(OC).*-\s([0-9]+)/";
            $expQtd = "/([0-9]){1,4},([0-9]{4})/";
            $expVlr = "/([0-9]{2}|[0-9]{1}),([0-9]{2})/";

            $resultArtigos = array();
            $resultOC = array();
            $resultQtd = array();
            $resultVlr = array();
            preg_match_all($expArtigos, $dados, $resultArtigos);
            preg_match_all($expOC, $dados, $resultOC);
            preg_match_all($expQtd, $dados, $resultQtd);
            preg_match_all($expVlr, $dados, $resultVlr);

            $newdata = '';
            $auxVlr = $j = $i = 0;

            foreach($resultArtigos[0] as $a) {
               echo $resultQtd[0][$i] . "\n";
               $newdata .= $a."|".$resultOC[0][$i]."|".$resultQtd[0][$i]."|".$resultVlr[0][$i]."\n";
               $i++;
//                if(!(strpos($a, 'ACRESCIMOS') > 0)) {
//                    while($resultVlr[0][$j] < 15) {
//                        $auxVlr++;
//                        $j++;
//                    }
//                    $newdata .= $a."|".$resultOC[0][$i]."|".$resultQtd[0][$i]."|".$resultVlr[0][$j].";";
//                    $i++;
//                    $j = $i + $auxVlr;
//                }
            }
            ...
            var_dump($filename);
            $copia = fopen($filename,'w');
            fwrite($copia, $newdata);
            fclose($copia);
        }
